...
import { Activity, RefreshCw, Search, SlidersHorizontal, Play, X, Key, Eye, EyeOff, ChevronDown, ChevronRight, CheckCircle2, XCircle, AlertCircle, Clock, Globe } from 'lucide-react';
...
function PingTableRow({ app, result, loading, selected, onToggle, expandedId, setExpandedId }) {
...
export default function PingTestPage() {
  ...
  const loadApps = async (bgId) => {
    setAppsLoading(true); setApps([]); setResults({});
    try {
      ...
      ar.forEach((r, i) => { if (r.status === 'fulfilled') (r.value.data.data || []).forEach(a => { const k = `${a.id}|${a.environment?.id||''}`; if (!sa.has(k)) { sa.add(k); ma.push({ ...a, _bgId: bgIds[i] }); } }); });
      er.forEach(r => { if (r.status === 'fulfilled') (r.value.data.data || []).forEach(e => { if (!se.has(e.id)) { se.add(e.id); me.push(e); } }); });
  ...
  const appsToTest = useMemo(() =>
    selectedAppIds.size > 0 ? filtered.filter(a => selectedAppIds.has(`${a.id}|${a.environment?.id}`)) : filtered,
  ...
  const displayApps = useMemo(() => {
    ...
    if (showTestedOnly) {
      ...
      list = [...src.filter(a => results[a.id])].sort((a, b) => (ord[results[a.id]?.status] ?? 3) - (ord[results[b.id]?.status] ?? 3));
    } else {
      list = filtered;
    }
    // Selected apps float to the top
    if (selectedAppIds.size > 0) {
      return [...list].sort((a, b) => {
        const aKey = `${a.id}|${a.environment?.id}`;
        const bKey = `${b.id}|${b.environment?.id}`;
        const aSelected = selectedAppIds.has(aKey) ? 0 : 1;
        const bSelected = selectedAppIds.has(bKey) ? 0 : 1;
    ...
    for (const app of appsToTest) {
      ...
      try {
        const { data } = await api.post('/health/ping', {
          targetType: isCH1 ? 'CH1' : 'CH2', appName: app.name, ch2IngressUrl,
  ...
  const handleRun = async () => { setShowTestedOnly(false); await runAllPings(); setShowTestedOnly(true); };
  ...
  const envOptions = [{ value: '', label: 'All Environments' }, ...environments.map(e => ({ value: e.id, label: e.name, badge: true, badgeColor: ENV_BADGE[e.type] || 'bg-gray-400', tag: e.type, tagColor: ENV_TAG_COLOR[e.type] || 'bg-gray-700 text-gray-400' }))];
  const typeOptions = [{ value: '', label: 'All Types' }, { value: 'CloudHub 2.0', label: 'CloudHub 2.0', tag: 'CH2', tagColor: 'bg-blue-500/20 text-blue-400' }, { value: 'CloudHub 1.0', label: 'CloudHub 1.0', tag: 'CH1', tagColor: 'bg-purple-500/20 text-purple-400' }];
  const done = Object.keys(results).length;
  const successCount = Object.values(results).filter(r => r.status === 'SUCCESS').length;
  const partialCount = Object.values(results).filter(r => r.status === 'PARTIAL').length;
  const failedCount  = Object.values(results).filter(r => r.status === 'FAILED').length;
  const testLabel = selectedAppIds.size > 0 ? `Run (${selectedAppIds.size} selected)` : `Run Ping Tests (${filtered.length})`;

  return (
    <div className="space-y-5">
      {showBgFilter && <BgFilterModal businessGroups={allBusinessGroups} onClose={() => setShowBgFilter(false)} onSaved={() => { const v = applyBgFilter(allBusinessGroups); if (selectedBg !== '__all__' && !v.find(g => g.id === selectedBg)) setSelectedBg('__all__'); }} />}

      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><Activity size={20} className="text-cyan-400" /> Ping Test</h1>
          <p className="text-gray-400 text-sm mt-1">
            {filtered.length} apps{done > 0 && <span className="ml-2">· <span className="text-emerald-400">{successCount} ✓</span>{partialCount > 0 && <span className="text-yellow-400 ml-1">{partialCount} ~</span>}{failedCount > 0 && <span className="text-red-400 ml-1">{failedCount} ✗</span>}</span>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {running ? <button onClick={() => setRunning(false)} className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-red-700 hover:bg-red-600 text-white rounded-lg"><X size={14} /> Stop</button>
            : <button onClick={handleRun} disabled={appsLoading || filtered.length === 0} className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-cyan-700 hover:bg-cyan-600 disabled:opacity-50 text-white rounded-lg"><Play size={14} /> {testLabel}</button>}
          <button onClick={() => { setResults({}); setExpandedId(null); }} disabled={done === 0} className="flex items-center gap-2 px-3 py-2 text-sm text-gray-400 hover:text-white bg-gray-800 rounded-lg disabled:opacity-40"><RefreshCw size={13} /> Clear</button>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={() => setShowBgFilter(true)} className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded border ${filterActive ? 'bg-blue-600/20 border-blue-600/50 text-blue-400' : 'bg-gray-800 border-gray-700 text-gray-500 hover:text-gray-300'}`}><SlidersHorizontal size={9} />{filterActive ? `${visibleGroups.length}/${allBusinessGroups.length}` : 'Filter'}</button>
            </div>
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search app names..." className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-9 pr-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-cyan-600/50" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1 border-t border-gray-800">
          <div>
            <label className="text-[10px] text-gray-500 uppercase tracking-wider font-medium flex items-center gap-1 mb-1"><Key size={9} /> client_id</label>
            <input value={clientId} onChange={e => setClientId(e.target.value)} placeholder="optional" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-xs text-gray-200 font-mono focus:outline-none focus:border-cyan-600/50" />
          </div>
          <div>
            <label className="text-[10px] text-gray-500 uppercase tracking-wider font-medium flex items-center gap-1 mb-1"><Key size={9} /> client_secret</label>
            <div className="relative">
              <input value={clientSecret} onChange={e => setClientSecret(e.target.value)} type={showSecret ? 'text' : 'password'} placeholder="optional" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 pr-8 text-xs text-gray-200 font-mono focus:outline-none focus:border-cyan-600/50" />
              <button onClick={() => setShowSecret(!showSecret)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300">{showSecret ? <EyeOff size={12} /> : <Eye size={12} />}</button>
            </div>
          </div>
          <div>
            <label className="text-[10px] text-gray-500 uppercase tracking-wider font-medium mb-1 block">x-transaction-id</label>
            <input value={transactionId} onChange={e => setTransactionId(e.target.value)} placeholder="smokeTest" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-xs text-gray-200 font-mono focus:outline-none focus:border-cyan-600/50" />
          </div>
        </div>
      </div>
      ...
      {running && (
        <div className="bg-gray-900 border border-cyan-800/40 rounded-xl px-5 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between text-xs mb-1.5"><span className="text-cyan-300 font-medium">Running…</span><span className="text-gray-500">{done} / {appsToTest.length}</span></div>
            <div className="w-full bg-gray-800 rounded-full h-1.5"><div className="bg-cyan-500 h-1.5 rounded-full transition-all" style={{ width: `${appsToTest.length > 0 ? (done / appsToTest.length) * 100 : 0}%` }} /></div>
          </div>
        </div>
      )}
      ...
      {!running && done > 0 && (
        <div className="flex items-center justify-between flex-wrap gap-3 px-5 py-3 bg-gray-900 border border-gray-800 rounded-xl text-sm">
          ...
          <button onClick={() => setShowTestedOnly(!showTestedOnly)}
            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${showTestedOnly ? 'bg-cyan-600/20 border-cyan-600/60 text-cyan-300' : 'bg-gray-800 border-gray-700 text-gray-400 hover:text-white'}`}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-800/50 text-gray-400 text-xs uppercase tracking-wider">
                <th className="px-4 py-3 w-10" onClick={() => allFilteredSelected ? setSelectedAppIds(new Set()) : setSelectedAppIds(new Set(filtered.map(a => `${a.id}|${a.environment?.id}`)))}>
                  <div className={`w-4 h-4 rounded border flex items-center justify-center cursor-pointer ${allFilteredSelected ? 'bg-cyan-600 border-cyan-500' : someSelected ? 'bg-cyan-900/60 border-cyan-600' : 'border-gray-600 hover:border-cyan-500'}`}>
      ...
      {!appsLoading && displayApps.length === 0 && apps.length > 0 && !showTestedOnly && (
        <div className="flex flex-col items-center justify-center py-12 gap-3">
          ...
          <button onClick={() => { setSearch(''); setFilterEnv(''); setFilterType(''); }} className="text-cyan-400 hover:text-cyan-300 text-xs underline">Clear filters</button>
        </div>
      )}
    </div>
  );
