          ...
          {attempts.map((a, i) => {
            ...
            return (
              <div key={i} className="border-b border-gray-800/30 last:border-0">
                <button
                  onClick={() => a.payload ? setExpandedIdx(isExpanded ? null : i) : undefined}
                  className={`w-full flex items-center gap-2 px-3 py-2 text-[11px] text-left transition-colors ${
                    a.payload ? 'hover:bg-gray-800/30 cursor-pointer' : 'cursor-default'
                ...
                {isExpanded && a.payload && (
                  <pre className={`px-3 py-2.5 text-[11px] overflow-auto font-mono border-t leading-relaxed whitespace-pre-wrap break-all max-h-48 ${
                    has5xx ? 'bg-red-950/20 border-red-800/30 text-red-300/80' : 'bg-[#0B0F17] border-gray-800/40 text-emerald-400/80'
                  }`}>
